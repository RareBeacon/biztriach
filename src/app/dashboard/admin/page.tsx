  ...
  return(
    <div className="space-y-6">
      ...
      <div className="rounded-[20px] bg-white border shadow-soft overflow-hidden">
        <div className="p-5 border-b flex justify-between items-center"><h3 className="font-semibold text-[14px] flex items-center gap-2"><Users className="w-4 h-4"/> All Users - Manual Billing Flow</h3><div className="flex gap-2">{["ALL","PENDING","APPROVED","SUSPENDED","REJECTED"].map(f=><button key={f} onClick={()=>setFilter(f)} className={`h-7 px-3 rounded-full text-[11px] font-medium border ${filter===f?"bg-violet-600 text-white":"bg-white hover:bg-slate-50"}`}>{f}</button>)}</div></div>
